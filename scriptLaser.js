function createAndPositionDiv(elementClass, x, y, width, height, colour) {
    ...
    if (newDiv.className === 'laserTrail') {
...
window.addEventListener('resize', () => {
    document.querySelectorAll('.laserTrail').forEach(element => {
        element.remove()
    });
    document.querySelectorAll('.laser').forEach(element => {
        element.remove()
    });
});
